...
dag = DAG(
    ...
    start_date=datetime(year=2021, month=4,day=7,hour=10,second=1),
...
task_duplicate = BashOperator(
    task_id="task_duplicate",
    bash_command='echo duplicate ',
...
task_zoneAirflow_failure = BashOperator(
    ...
    # Must have a space
    bash_command='python3 /opt/airflow/dags/task_upload_failure.py "{{ prev_execution_date }}" "zoneAirflow" ',
    trigger_rule='one_failed',
    retries = 2,
    dag=dag)
...
task_zoneTemperature_failure = BashOperator(
    ...
    # Must have a space
    bash_command='python3 /opt/airflow/dags/task_upload_failure.py "{{ prev_execution_date }}" "zoneTemperature" ',
    trigger_rule='one_failed',
    retries=2,
    dag=dag)
